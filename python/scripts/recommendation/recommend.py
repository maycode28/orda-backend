...
def _load_monthly_popular_courses(conn) -> pd.DataFrame:
    sql = f"""
        ...
        SELECT
            cf.*,
            COALESCE(mp.hiking_count, 0) AS hiking_count,
            COALESCE(mp.summit_completion_count, 0) AS summit_completion_count
...
def _popular_or_fallback_courses(
    conn,
...
def recommend(user_id: int, top_n: int = 5) -> list[dict]:
    ...
    with _connect() as conn:
        ...
        if user_history.empty:
            return [item.to_dict() for item in _popular_or_fallback_courses(conn, courses, top_n)]
